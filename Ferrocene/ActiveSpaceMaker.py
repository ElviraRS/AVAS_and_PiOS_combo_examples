...
def MakeActiveSpace(mol,mf):
   ...
   # indices are 1-based.
   PiSystem1 = [2,3,4,5,6] # this is your Cp ring #1
   PiSystem2 = [12,13,14,16,18]         # this is your Cp ring #1
   Atoms4AVAS = [1]  #atoms for AVAS
...
#3) you have a charge on your pi-system. For ferrocene you should assign one more electron to one of C in your pi-systems
   ...
   AssignTag(Elements,2, "C", "2e")
   AssignTag(Elements,12, "C", "2e")
   ...
   # add pi-orbitals for PiSystem1

   CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakePiSystemOrbitals("PiSystem1", PiSystem1, None, Elements,Coords, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)
   for i in range(1,nOccOrbExpected+1):
      CActOcc.append(CFragOcc[:,-i])
   for j in range(nVirtOrbExpected):
      CActVir.append(CFragVir[:,j])

   # add pi-orbitals  for PiSystem2
   CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakePiSystemOrbitals("PiSystem2", PiSystem2, None, Elements,Coords, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)
   for i in range(1,nOccOrbExpected+1):
      CActOcc.append(CFragOcc[:,-i])
   for j in range(nVirtOrbExpected):
      CActVir.append(CFragVir[:,j])

#   CActOcc.append(CFragOcc[:,-2]) if you want just selected HOMOs and LUMOs, then add them in this manner
#   CActOcc.append(CFragOcc[:,-1])
#   CActVir.append(CFragVir[:,0])
#   CActVir.append(CFragVir[:,1])

   # add AVAS for metal centers
   for iAt in Atoms4AVAS:
      CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakeAvas("Fe {}".format(iAt), iAt, 2, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)     # L=2 for d-orbitals
   for i in range(1,nOccOrbExpected+1):
      CActOcc.append(CFragOcc[:,-i])
   for j in range(nVirtOrbExpected):
      CActVir.append(CFragVir[:,j])
