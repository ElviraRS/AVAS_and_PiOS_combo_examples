...
''' When using results of this code for publications, please cite the following paper:
    ...
    "Automatic construction of molecular active spaces from atomic valence orbitals"
     E. R. Sayfutyarova, Q. Sun, G. K.-L. Chan, G. Knizia, J. Chem. Theory Comput., 13, 4063 (2017).
...
def MakeActiveSpace(mol,mf):
   ...
   # PiAtoms lists contain the set of main-group atoms involved in the pi-system 
   # indices are 1-based.
   # note two O atoms are not part of ring, but their lone pairs are clearly delocalized into ring.  
   FmnPiAtoms = [42,43,44,45,47, 48,49,50,51,52, 54,59,64,66,67, 68]
   TrpPiAtoms = [27,28,30,32,33,34,36,38,40]
   TyrPiAtoms = [4,5,7,9,12,14]
   GlnPiAtoms = [19,20,21]

   TransferringHydrogenAtoms = [22, 11]   #atoms for AVAS
...
#3) you have a charge on your pi-system. For ferrocene you should assign one more electron to one of C in your pi-systems
   ...
   AssignTag(Elements,50, "N", "1e")
   AssignTag(Elements,10, "O", "2e")
   AssignTag(Elements,20, "O", "1e")
   AssignTag(Elements,21, "N", "2e")
   ...
   # add pi-orbitals for Fmn residue
   CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakePiSystemOrbitals("Fmn", FmnPiAtoms, None, Elements,Coords, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)
   #elected 2 HOMOs and 1 LUMO
   CActOcc.append(CFragOcc[:,-2])
   CActOcc.append(CFragOcc[:,-1])
   CActVir.append(CFragVir[:,0])


   # add pi-orbitals for Trp residue
   CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakePiSystemOrbitals("Trp", TrpPiAtoms, None, Elements,Coords, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)
   #elected 2 HOMOs and 1 LUMO
   CActOcc.append(CFragOcc[:,-2])
   CActOcc.append(CFragOcc[:,-1])
   CActVir.append(CFragVir[:,0])

   # add pi-orbitals for Tyr residue
   CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakePiSystemOrbitals("Tyr", TyrPiAtoms, None, Elements,Coords, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)
   #elected 2 HOMOs and 1 LUMO
   CActOcc.append(CFragOcc[:,-2])
   CActOcc.append(CFragOcc[:,-1])
   CActVir.append(CFragVir[:,0])

   # add pi-orbitals for Gln residue
   CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakePiSystemOrbitals("Gln", GlnPiAtoms, None, Elements,Coords, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)
   #elected 1 HOMO and 1 LUMO
   CActOcc.append(CFragOcc[:,-1])
   CActVir.append(CFragVir[:,0])


   # add AVAS for transferring hydrogens
   for iAt in TransferringHydrogenAtoms:
      CFragOcc, CFragVir,nOccOrbExpected,nVirtOrbExpected = MakeAvas("H {}".format(iAt), iAt, 0, CIb, Shells, S1, S12, S2, Fock, COcc, CVir)     # L=0 for s-orbitals
...
def dot2(A,B): return np.dot(A.flatten(),B.flatten())
...
def MakeAvas(TargetName, iAt_, TargetL,  CIb, Shells, S1, S12, S2, Fock, COcc, CVir):
   print("\n *** TARGET: {}\n".format(TargetName))
   iAt = iAt_ - 1 # convert from 1-based to 0-based
    # L=2 for d-orbitals
   iFnS = FindValenceAoIndices(iAt, Shells, TargetL = TargetL)
   assert(iFnS is not None and len(iFnS) == 2*TargetL+1)

   CTargetIb = CIb[:,np.array(iFnS)].copy()
   nTargetIb = CTargetIb.shape[1]
   print ("Number of target MINAO basis fn", nTargetIb)
  
   nOccOrbExpected=nTargetIb
   nVirtOrbExpected=nTargetIb
   CAvasOcc,nOccOrbExpected  = MakeOverlappingOrbSubspace("AVAS", "Occ", COcc, nOccOrbExpected,   CTargetIb, S1, Fock)
   CAvasVir,nVirtOrbExpected = MakeOverlappingOrbSubspace("AVAS", "Vir", CVir, nVirtOrbExpected,   CTargetIb, S1, Fock)
   ...
   return CAvasOcc, CAvasVir,nOccOrbExpected,nVirtOrbExpected
